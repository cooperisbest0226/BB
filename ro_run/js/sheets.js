...
const CHANGELOG=[
  ...
  { v:'v64', d:'2026/09/07',
    note:'架構健檢：資料愈累積愈慢的幾個地方都修掉了，操作行為完全沒變。',
    c:[
    ['imp','拍賣頁與材料頁的重繪速度提升約四倍。原本每次改個數字都要把千分位格式器重建上千次、把「全部場次」的索引重算幾百遍，算的都是同一個答案'],
    ['imp','在拍賣頁編輯時不再重畫看不到的材料頁。以前為了取得「目前可組成幾組」這一個數字，整頁的長條圖與場次明細都會跟著重建'],
    ['chg','成交紀錄預設顯示最近三個月，按鈕會標明還有幾個月幾筆可以往前看。這份帳只會愈來愈長，每次操作都全部重建會愈用愈卡'],
    ['fix','背景更新真的會抓到新版了。以前用快取回應之後 Service Worker 就可能被休眠，背景抓新版的請求跟著被中斷'],
    ['fix','程式出錯時會跳出提示並說明資料仍在，不再是「按了沒反應」然後靜靜卡住'],
    ['imp','字型改成不擋畫面：訊號差或離線第一次開啟時，App 會先出來，字型晚到再換上'],
    ['fix','分頁圖示補進離線預先快取；標題列補上頁面標題，輔助工具讀得出這一頁是什麼'],
  ]},
  { v:'v63', d:'2026/09/05',
    c:[
    ['fix','材料頁的「已售出組數」會拆開標示台幣與 R 幣各賣了幾組。這一頁兩種幣別都算（賣掉一組就是賣掉一組），但成交紀錄一次只顯示一種，數字對不起來時無從查證'],
    ['imp','同一批貨若兩種幣別各記了一次，卡片會直接提示回成交紀錄切換幣別查看重複的那筆'],
    ['fix','分潤已標記領取的那一列，標籤與按鈕改成可換行，金額大的時候不會擠成一團'],
  ]},
  { v:'v62', d:'2026/09/05',
    note:'「已領」改成累積扣除。先發一次、之後再看更大的期間時，要直接看到「還差多少」，而不是重新看到一次全額。',
    c:[
    ['chg','分潤每一列的主要數字改成「待領」＝應得 − 期間內已領。9/1 先發了 5000W，之後選全部日期會直接顯示扣掉之後的數字'],
    ['add','列上標出「應得 X · 已領 Y」，對帳時追得回來待領是怎麼算出來的'],
    ['add','標記已領時記下的是「當下還差的金額」；之後補記交易讓應得變高，差額會自動變成新的待領'],
    ['add','跟目前期間只有部分重疊的結算不扣除（無法判斷該扣多少），但會在該列標示出來'],
    ['fix','可組成組數會扣掉已經賣出的材料。整組交易按配方換算回材料，單品交易直接扣——賣完之後不會再帶入舊的組數'],
    ['imp','材料結果卡寫出扣除過程：掉落夠組 N 組、已售出 M 組、還能組 X 組'],
  ]},
  { v:'v61', d:'2026/09/05',
    note:'分潤算出來只是第一步，錢還要一個一個發。中間最常斷掉的是「我發到誰了」。',
    c:[
    ['add','分潤試算每個人可以標記已領，摘要顯示「幾人已領 / 待發金額」，全部發完會另外提示'],
    ['add','發錢之後才補記交易的話，該成員會顯示「已發 X，差 Y」——實際發出去的金額不會被之後的試算改寫'],
    ['add','匯出圖片也標示已領，貼到群組讓大家自己看，不用一個一個回答'],
    ['add','日期快捷鈕（今天／本週／本月／全部）會標出目前選中的那顆；手動挑的日期四顆都不亮'],
    ['fix','記錄交易後自動清空組數、每組價格與歸屬場次，並收起場次清單——留著舊的歸屬最容易在沒注意時記錯下一筆'],
    ['imp','四個篩選面板的日期快捷鈕改用同一份邏輯，不再各自維護一份'],
    ['imp','資料結構升到第 11 版，新增領取紀錄'],
  ]},
  { v:'v60', d:'2026/09/04',
    note:'公基金整個拿掉。翻車就是沒打成功、沒有掉落物，沒有東西可以分——不是「先扣起來放公基金」。收入現在全數分完。',
    c:[
    ['rm','移除公基金。除不盡的差額改用最大餘額法一元一元發完，每人金額加總精確等於總收入'],
    ['chg','翻車場不再吃掉一份。未指定歸屬時只攤給通關且有人出場的場次——五場翻一場，那一場不會拿走五分之一'],
    ['chg','指到翻車場的收入會退回當天其他通關場次；當天沒有別場才列為未歸屬'],
    ['fix','分潤的日期區間改成篩「場次日期」而不是「交易日期」。九月賣掉八月打的材料，算在八月那幾場'],
    ['fix','未指定歸屬又找不到可分潤場次的交易會跳警示並說明怎麼修，不再靜靜消失在畫面上'],
    ['fix','分潤頁沒有資料時匯出鈕沒收起來（自訂 display 蓋掉了瀏覽器預設的 hidden）'],
    ['imp','摘要與匯出圖改成「總收入 · 幾場有收入 · 幾人分潤」，驗算方式就是把每人金額加起來'],
  ]},
  { v:'v59', d:'2026/09/04',
    c:[
    ['fix','歸屬場次改成可跨天複選。材料本來就是累積好幾場、好幾天才一次賣掉，原本只列「今天」的場次，賣出當天沒排場次就整個是空的'],
    ['chg','編輯既有交易時用同一個複選器，可以回頭修正歸屬場次'],
    ['chg','一筆交易的錢平均攤到選中的每一場，再各自分給那場的人'],
    ['imp','「通關 N 場」改成「分潤 N 場」，算的是實際分到錢的場次，跟旁邊的金額對得起來'],
    ['imp','資料結構升到第 10 版。原本綁單一場次的紀錄會自動包成清單，不用重設'],
  ]},
  { v:'v57', d:'2026/09/04',
    c:[
    ['imp','統計卡標籤簡化成「累計總額」「平均每組」，不再重複頁面上方已經標明的幣別'],
  ]},
  { v:'v56', d:'2026/09/04',
    note:'一筆交易本來就只會是「賣台幣」或「賣 R 幣」其中一種，兩欄並存等於每筆都要填一個換算率。',
    c:[
    ['chg','「台幣（每組）」與「遊戲 R 幣值」合併成幣別切換加單一價格欄位，依當初實際怎麼成交的記錄'],
    ['add','拍賣頁加幣別切換，統計、走勢、行情、成交紀錄、分潤全部跟著切'],
    ['chg','分潤依幣別各自成池、各自平帳。沒有換算率就不會有捏造的匯率混進分錢的算式'],
    ['rm','移除台幣兌 R 幣的換算率欄位與「累計總 R 幣」統計卡'],
    ['imp','資料結構升到第 9 版。舊紀錄一律先當台幣，可在編輯交易時逐筆改成 R 幣'],
  ]},
  { v:'v55', d:'2026/09/04',
    note:'算錢的東西要能被驗算，所以金額全程用「分」做整數運算——浮點數在七位數除以三個人時會少一塊錢，而且錯得沒有規律。',
    c:[
    ['add','拍賣頁新增「分潤試算」子分頁：選一段期間，算出每人金額，可匯出圖片貼群組'],
    ['add','記錄交易時可指定歸屬場次，同一天多場、成員不一樣時才分得出這筆錢該給誰'],
    ['imp','資料結構升到第 8 版'],
  ]},
  { v:'v54', d:'2026/09/04',
    c:[
    ['add','成員頁新增「出場統計」子分頁：可選期間，列出每人的出場、通關、翻車次數與成功率'],
    ['add','雙層長條同時表達三個數字——軌道長度是出場，填滿的部分是通關，露出的尾巴就是翻車'],
    ['add','缺席名單獨立列出。只列有出場的人的話，「這段期間誰完全沒排到」剛好變成最看不見的資訊'],
  ]},
  { v:'v53', d:'2026/09/04',
    c:[
    ['add','RUN 卡片可標記翻車，點標題旁的徽章切換；翻車的卡片左緣加紅邊，滑長列表時不用逐張讀'],
    ['add','匯出圖片、CSV 與材料頁的場次明細都會標出翻車'],
    ['chg','複製 RUN 不再繼承翻車標記，跟掉落物、錄影連結同一個道理'],
    ['imp','資料結構升到第 7 版。遷移前的舊場次一律視為通關，所以早期成功率會偏高，等新資料累積才會回到真實值'],
  ]},
  { v:'v52', d:'2026/08/21',
  ...
  { v:'v49', d:'2026/08/19', c:[
    ['chg','材料分成四類各自給色：碎片藍、浮塵青、未知紫、稀微綠 —— 未知的隕石碎片與稀微魔力符文石原本都被歸進同一堆灰色，兩種完全不同的東西長得一樣'],
    ['chg','組數試算預設改為「每種材料 1 個 = 1 組」'],
    ['add','場次明細加入「全部展開 / 全部收合」，並顯示天數與場數摘要'],
    ['fix','系列的預設歸類寫死了陣列索引，系列數量一變就會指到錯的顏色'],
  ]},
  { v:'v48', d:'2026/08/19',
  ...
  { v:'v45', d:'2026/08/12', c:[
    ...
    ['fix','瓶頸的琥珀色在深色模式下太暗，改用會跟著主題調整的色票'],
  ]},
