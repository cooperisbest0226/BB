...
Phase 4（v1.4.0）。純前端、無後端、資料存在瀏覽器 IndexedDB。
...
大螢幕上行事曆格子太小，主因是 `.page` 的 960px 寬度上限
（那是為閱讀取向的頁面訂的，行事曆是網格，需要寬度）。三項調整：

- **Calendar 與 Timeline 跳出 960px 限制**，上限改為 1500px，其餘頁面不變。
  每欄寬度：1440px 螢幕 95 → 131px，1920px 螢幕 95 → 172px
- **格子高度 96 → 132px**，每格可顯示的任務從 3 件提高到 4 件
- **字級調大**：任務 11.5 → 12.5px、日期 12 → 13px、星期列 11.5 → 12px

手機版維持原本的圓點模式，不受影響。

順帶補上**拖曳邊緣自動捲動** —— 格子加高後，一個月的月曆會比視窗還高，
原本無法把任務拖到看不見的日期。現在游標靠近上下緣會自動捲動。

## v1.3.1 —— 壓力測試後的修正

跑了一輪大規模與邊界壓力測試（5000 任務 / 100 專案、極端輸入、注入、
損壞備份、300 次連續操作、五種時區、五種視窗尺寸），修掉四個問題：

- **超長標題會撐爆版面**：`.row-title` 等元素是 `<span>`，而 `overflow:hidden`
  對 inline 元素無效，省略號從來沒生效過。已補 `display:block`
- **專案顏色欄位可被注入 CSS**：`color` 原本直接插進 `style` 屬性。
  手改或損壞的備份檔可以塞進任意 CSS。已加 `safeColor()` 驗證，
  非合法 hex 一律退回預設色，匯入時也會正規化
- **看板拖拉在 Chrome 其實是壞的**：`dragstart` 沒有呼叫 `setData()`，
  Chrome 會直接取消拖曳。行事曆的拖拉有做所以正常，看板漏了
- **儲存失敗會被靜默吞掉**：配額用盡或無痕模式下寫入失敗時，
  畫面仍顯示改過的資料，但其實沒存進去。現在會提示並把畫面
  還原成資料庫的真實狀態

另外把看板欄加上高度上限與內部捲動 —— 幾百張卡時整頁會被撐到幾千 px，
跨欄拖拉實務上做不到。
